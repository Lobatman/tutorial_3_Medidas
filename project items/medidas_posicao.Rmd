...
kable(tabela_formatada, align = "c") %>%
  ...
  kable_styling(bootstrap_options = "condensed")
```
A tabela acima, representa o script utilizado no exemplo do tutorial anterior, para criar uma tabela de frequências a partir da IDE base do R tendo como variável ``tabela_freq_albumina``. Agora, a partir deste código, vamos realizar o nosso cálculo da média para dados agrupados em classes deste exemplo:  
...
* Na variável ``xi <- (brk[-1] + brk[-length(brk)]) / 2``, como o nome dado a variável diz, aqui nós descobrimos o *ponto médio* (xi) das classes. Utilizamos ``brk[-1]`` para remover o primeiro elemento da variável ``brk``, lembrando que ``brk`` representa a sequência de valores que contém os limites de cada classe na tabela de frequência. Ao adicionar ``[-1]`` em nosso código, nós removemos o limite inferior da primeira classe.  
  
  + Ainda na primeiro variável, seguindo nosso código, em ``brk[-length(brk)]``, nós removemos o último elemento da variável ``brk``, ou seja, o limite superior da última classe. Removemos o limite inferior da primeira classe em ``brk[-1]`` e o limite superior em ``brk[-length(brk)]`` para separamas cada uma das classes e respectivamente encontrar o ponto médio.  
...
* Na variável ``media_agrupada <- sum(xi * tabela_freq_albumina) / sum(tabela_freq_albumina)``, foi criado para calcular a média ponderada dos pontos médios das classes (xi) e das frequências absoluta (fi) ou seja, utilizamos para calcular *fixi*. em ``xi`` nós temos o ponto médio de cada classe, e em ``tabela_freq_albumina`` nós temos as frequências relativas correspondentes a cada uma das classes da tabela. Tendo em vista isso, foi feito a multiplicação de xi e fi, compondo o código com ``(xi * tabela_freq_albumina)``.  
  + Após multiplicar *fixi*, foi feito a soma dos resultados através da função ``sum()``, portanto: ``sum(xi * tabela_freq_albumina)``.  
  
  + Por fim, foi feito a divisão desta soma pelo total das frequências relativas (fi), finalizando esta variável: ``media_agrupada <- sum(xi * tabela_freq_albumina) / sum(tabela_freq_albumina)``  
...
```
Neste exemplo, conseguimos simplificar um pouco os métodos para obter a média para dados agrupados em classes. Perceba que, ainda precisamos criar a variável ``xi <- (brk[-1] + brk[-length(brk)]) / 2``, porém podemos simplificar a segunda parte, utilzando a função ``weighted.mean()``, adicionando a variável ``tabela_freq_albumina`` que representa o nosso **fi**. O resultado final será o mesmo. 
...
Para calcular a média geométrica, vamos primeiro utilizar a variável dos dados referente ao número de infectados, e em seguida criar uma variável para a média geométrica:  
...
media_geom
```
* Utilizamos a função ``prod()`` para multiplicar os números contidos dentro da variável **n_infectados**.
  + Em seguida, utilizamos o sinal de **^** para elevar o produto de todos os dados calculados na variável n_infectados.;
  + De acordo com a fórmula, precisamos dividir o resultado do produto por $\frac{1}{n}$, portanto, criamos a função ``(1/length(n_infectados))``, onde ``length()`` irá retornar para nós o número total de elementos contidos na variável **n_infectados**, portanto, ao escrevermos ``1/length(n_infectados)`` estamos dividindo 1 pelo número total de dados contidos na variável **n_infectados**  
...
Suponha que você precisa calcular a média móvel da qantidade de óbitos por covid em oito dias de determinado local, de acordo com a tabela:  
...
A partir destes dados, vamos calcular a média móvel, considerando que a variável da tabela é ``tabela_MM``:  
...
Agora que criamos nossa variável (``mortos``), que possui todos os dados referentes aos óbitos na tabela, precisamso calcular a média móvel.  
...
* **x** é a variável que possui os dados para calcular a média móvel, que nesse caso é **mortos**.  
...
Para a criação do nosso gráfico, utilizamos a biblioteca **ggplot2** e criamos uma variável com nome de **observacoes** onde atribuimos a ela valores de 1 até o comprimento máximo do vetor **mortos**, tendo então:``observacoes <- 1: length(mortos)``.  
...
O próximo passo para plotar o gráfico, é criar o título do gráfico e os rótulos dos eixos (x e y). Para isso, utilizamos a função ``labs()``, que é utilizada para definir o título ("title") de um gráfico, dessa forma temos a seguinte linha de cógito: ``labs(title = "Óbitos por COVID-19", x = "Dias", y = "Óbitos", color = "Legenda")``.  Atribuimos o nome desejado entre " " para cada um dos termos *title*, *x* e *y* que aparecerão no gráfico. 
  
Utilizamos a função ``scale_color-manual()`` que é usada para definir as cores da legenda. Em nosso gráfico, utilizamos a cor "red" para indicar a Média Móvel que está em vermelho, e a cor "black" para indicar os Óbitos que está representado de preto.  
...
Para calcular a média harmônica, primeiro nós criamos uma variável com nome de ``reciproco``, e dentro dele atribuimos ``1 / n``, para calcular o reciproco de cada um dos dados contidos no vetor *n*, armazenando o resultado dentro deste vetor.  
Em seguida, criamos a variável ``media_harm``. nesta variável criamos ``length(n) / sum(reciproco)``.  Em ``length(n)`` nós retornamos o tamanho do vetor *n* e dividimos pela soma dos valores contidos na variável *reciproco*.  
O resultado é o valor da nossa média harmônica.  
  
# Mediana  
  
A mediana pode ser representada como um indicativo de qual valor se apresenta no meio de um conjunto de dados ordenados, ou seja, é o valor que vem precedido e sucedido do mesmo número de observações. A fórmula da mediana é apresentada por:  
...
Mas, para calcular a mediana para dados agrupados em classes, temos que descobrir o *Fant* que indica a frequência absoluta acumulada para baixo no intervalo anterior que contém a mediana. Para isso nós criamos uma nova variável com nome de **freq_abs_acumulada**.  
Nesta variável aplicamos a função ``cumsum()`` que é utilizada para calcular a soma acumulativa de elementos de determinado vetor ou tabela de frequências, tendo como resultado a frequência absoluta acumulada para baixo.
...
```
Agora, vamos criar as variáveis e funções para calcular a classe da mediana de acordo com as funções criadas, utilizdas para criar esta tabela de frequências (ver o tutorial anterior).  
...
```
O primeiro passo é, chamar novamente a variável que define a tabela de frequências do nosso exemplo: ``tabela_freq_albumina_t <- table(cut(qtd_albumina, breaks = brk, right = FALSE))``  
Em seguida, criamos ua variável para descobrir em qual intervalo contém a mediana, para isso temos: ``intervalo_mediana <- which.max(tabela_freq_albumina_t)``. Utilizamos a função ``which.max()`` para encontrar o índice que contém o valor máximo  na tabela de frequências (tabela_freq_albumina_t), correspoindente ao intervalo que contém a mediana.  
...
Porém, para criar um script que interprete isso, podemos utilizar a função ``as.numeric()`` para converter o nome do intervalo em um valor numérico, seguido de ``names()`` para conseguir os nomes dos intervalos na tabela de frequência.  
...
```
O n representa o tamanho da nossa amostra, e para descobri-la, criamos uma variável com a função ``sum()``, atribuida a variável *tabela_freq_albumina_t*, para que somasse todas as frequências absolutas da tabela de frequências.  
...
* A partir da função **subset()** e **table()**, nós criamos nosso script. No primeiro ``table(moda_a)``, utilizamo para criar a tabela;  
...
* Com ``subset`` que é utilizado para filtrar os dados, conseguimos selecionar apenas os dados que queremos, a a partir da condição que criamos para apresentar apenas os valores maiores. 
...
frequencia_modal
```
Um dos passos necessários, é criar uma variável para encontrar o indice modal, perceba que este passo também é igual ao feito no calculo da mediana para dados agrupados, a única diferença é que mudamos o vetor.  
...
frequencia_anterior
```
Encontrar a frequência anteriror, é necessária para identificar o valor de $\Delta1$ que corresponde a diferença entre a frequência da classe modal e a frequência da classe imediatamente anterior.  
...
frequencia_seguinte
```
Assim como no passo anterior, temos que criar uma variável para encontrar a frequência seguinte, que é necessário para encontrar o valor de $\Delta2$ que corresponde a diferença entre a frequência da classse modal e a frequência da classe imediatamente seguinte.  
...
plot_ep_s <- ggplot(data_medidas, 
     ...
     geom_errorbar(aes(ymin = Valor - erro_padrao_media, 
                       ymax = Valor + erro_padrao_media), 
                   width = 0.4) +
...
Lembrando que, essa á apenas uma opção simples de criar um boxplot, existem diversas outras forma de construção do mesmo, utilizando por exemplo a biblioteca *gggplot2*.  
